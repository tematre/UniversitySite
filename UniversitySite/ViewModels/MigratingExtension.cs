    ...
    public static class MigratingExtension
    {
            ...
            )
        {
            foreach (var professorVm in professorVmDic)
            {
                ...
                professorVm.Value.Subjects = professorDic[professorVm.Key].Subjects.Select(a => subjectVmDic[a.Id]).ToList();
            }
            ...
            foreach (var subjectViewModel in subjectVmDic)
            {
                ...
                subjectViewModel.Value.Students = subjectDic[subjectViewModel.Key].Students.Select(a => studentVmDic[a.Id]).ToList();
                subjectViewModel.Value.Professor = professorVmDic[subjectDic[subjectViewModel.Key].Professor.Id];
                subjectViewModel.Value.Marks = subjectDic[subjectViewModel.Key].Marks.Select(a => marksVmDic.ContainsKey(a.Id) ? marksVmDic[a.Id] : null).ToList();
            }
        ...
        public static ProfessorViewModel ToViewModel(this Professor that)
        {
            ...
            var studentVmDic = that.Subjects.SelectMany(a => a.Students).GroupBy(a=> a.Id).ToDictionary(a => a.Key, a => new StudentViewModel());
            var subjecVmtDic = that.Subjects.ToDictionary(a => a.Id, a => new SubjectViewModel());
            var marksVmDic = that.Subjects.SelectMany(a => a.Students).SelectMany(a => a.Marks).GroupBy(a => a.Id).ToDictionary(a => a.Key, a => new MarkViewModel());

            var studentDic = that.Subjects.SelectMany(a => a.Students).GroupBy(a => a.Id).ToDictionary(a => a.Key, a => a.FirstOrDefault());
            var subjectDic = that.Subjects.ToDictionary(a => a.Id, a => a);
            var marksDic = that.Subjects.SelectMany(a => a.Students).SelectMany(a => a.Marks).GroupBy(a => a.Id).ToDictionary(a => a.Key, a => a.FirstOrDefault());
        ...
        public static StudentViewModel ToViewModel(this Student that)
        {
            ...
            var professorDic = that.Subjects.Select(a => a.Professor).GroupBy(a => a.Id).ToDictionary(a => a.Key, a => a.FirstOrDefault());
            var professorVmDic = that.Subjects.Select(a => a.Professor).GroupBy(a => a.Id).ToDictionary(a => a.Key, a => new ProfessorViewModel());
